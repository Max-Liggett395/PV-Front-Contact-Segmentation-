...
# =====================================================================
CONFIG = {
    "architecture": "smp_pspnet",   # "unet", "deeplabv3", "deeplabv3plus", "smp_fpn", "smp_pspnet", "smp_manet", "smp_unetplusplus"
    "in_channels": 3,            # 1=grayscale, 3=RGB-stacked (3 required for ImageNet pretrained)
    "batch_size": 2,
    "max_epochs": 50,            # short budget for fast iteration
    "lr": 5e-4,
    "weight_decay": 0.0,
    "dropout": 0.3,
    "class_weights": None,       # e.g. [1.0, 1.0, 1.5, 1.0, 1.5, 1.5]
...
def create_model(config):
    ...
    elif arch == "smp_pspnet":
        import segmentation_models_pytorch as smp
        return smp.PSPNet(encoder_name="resnet50", encoder_weights="imagenet",
                          in_channels=in_ch, classes=n_cls)
    elif arch == "smp_manet":
        import segmentation_models_pytorch as smp
        return smp.MAnet(encoder_name="resnet34", encoder_weights="imagenet",
                         in_channels=in_ch, classes=n_cls)
    elif arch == "smp_unetplusplus":
        import segmentation_models_pytorch as smp
